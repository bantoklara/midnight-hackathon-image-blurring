...
type PublishState =
  ...
  | { status: "error"; message: string; needsFunds?: boolean };
 ...
 */
function describePublishError(err: unknown): { message: string; needsFunds: boolean } {
  const raw = err instanceof Error ? err.message : String(err);
  const needsFunds = /dust|insufficient|balance|not enough|funds|fee/i.test(raw);
  return { message: raw, needsFunds };
}
...
export default function TrueMaskApp() {
  ...
  async function publishRecord() {
    ...
    try {
      ...
      const receipt = await api.submitRedaction(pending.hash, pending.redaction);
      ...
      const described = describePublishError(err);
      setPublish({ status: "error", ...described });
    }
  }
